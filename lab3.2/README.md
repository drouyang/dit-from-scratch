...
CFG runs the model **twice per step** here — once with the prompt, once with the null embedding the model picked up during label-dropout training (lab 2.2 explains the mechanism). Production text-to-X engines generalize the null pass to a **negative prompt** (`""` by default; users can override to push generations away from things like *"blurry, low quality, distorted"*).
