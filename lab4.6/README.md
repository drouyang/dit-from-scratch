...
**Goal**: take what you trained in lab 4.4 and *publish it in three forms* each audience expects to install:
...
```
lab 4.4 produced:                        lab 4.6 turns it into:
─────────────────                        ─────────────────────────────
...
```bash
pip install -r lab4.6/requirements.txt
```
...
Upload from Python (`peft` already wrote both required files during training in lab 4.4):
...
api.upload_folder(
    folder_path="../lab4.4/runs/my-style/",     # contains .safetensors + adapter_config.json
...
```
1. trained a LoRA on a custom dataset                      (lab 4.4)
