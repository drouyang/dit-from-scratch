...
LoRA adapter, no parameter-efficient tricks. The trade-off vs lab 4.2:
...
The flow-matching forward pass is identical to lab 4.2. What's new is
...
the LoRA in lab 4.2 — full SFT is slower per step because every parameter
...
def main():
    ...
    # ── Frozen pretrained components: VAE + text encoder. Same as lab 4.2.
    ...
    # the only structural difference vs lab 4.2's LoRA loop.
    ...
    # ── Training loop. Identical math to lab 4.2 — only the optimizer's reach
...
def save_transformer(transformer, output_dir, step, accelerator):
    ...
    Unlike lab 4.2's LoRA save (~140 MB), this is the entire transformer
