...
**What multi-head attention buys you.** A single attention layer expresses only one routing pattern at a time. Multi-head splits the `D`-dim space into `H` independent subspaces of size `Dh = D / H`, each with its own `(Q_h, K_h, V_h)` projections. The `H` heads run their attention math entirely in parallel with no cross-talk — each is free to learn a different pattern. After attention finishes, the `H` outputs are **concatenated** (not selected) into one `D`-dim vector per position, and `W_O` learns how to weigh them per output feature. Think of it as consulting `H` independent experts who each look at the input from their own angle and hand back a `Dh`-dim summary; `W_O` is the learned blend across those summaries. Reverse only needs one routing pattern, so multiple heads typically converge to variants of the same anti-diagonal (you will see this in `visualize.py`); in a deep transformer on richer tasks, heads specialize on different relationships (syntax, coreference, long-range structure) — that emergent specialization is where multi-head pays off.
