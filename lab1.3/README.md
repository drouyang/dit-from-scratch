...
**How wide is attention's receptive field?** A CNN with a 3×3 kernel can only see 3×3 neighborhoods. Attention has no kernel size: for a length-`L` input the attention matrix is `L×L`, and every query can in principle attend to every key. The receptive field **is** the whole input — longer inputs produce wider attention matrices, and `O(L²)` compute and memory, the price of unrestricted reach (and why modern LLMs have context windows rather than infinite context). But "can attend to everything" is not "must attend to everything." In the reverse task, each learned attention row concentrates ~100% of its weight on a single key (`attn[i, L-1-i] ≈ 1`). In translation, a target word typically attends to a narrow band of 2-3 source words. Attention gives unlimited *maximum* reach; gradient descent decides how to spend it per task.
