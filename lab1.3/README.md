...
**What came before neural MT.** For roughly a decade, the dominant approach was **phrase-based statistical machine translation** (PBSMT) — systems like Moses that stitched together many hand-engineered components: a word-alignment model, a phrase translation table, a reordering model, a language model, and a log-linear combination tuned against BLEU. These worked, but they were pipelines. Errors propagated between components, and scaling required domain expertise.

**What RNN seq2seq solved (2014).** Sutskever et al. ("Sequence to Sequence Learning with Neural Networks") and Cho et al. ("Learning Phrase Representations using RNN Encoder-Decoder") showed you could replace the whole pipeline with one neural network trained end-to-end: a recurrent encoder reads the source sentence token-by-token into a final hidden vector, a recurrent decoder starts from that vector and generates the target sentence token-by-token. Train on parallel corpora with cross-entropy. No handwritten components. Sutskever's system hit competitive BLEU on WMT English→French. The paradigm — **neural machine translation** — was born here, and it was the first time one learned architecture matched a decade of engineered statistical MT.
